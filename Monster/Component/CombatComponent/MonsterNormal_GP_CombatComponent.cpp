...
#include "MonsterNormal_GP_CombatComponent.h"
...
UMonsterNormal_GP_CombatComponent::UMonsterNormal_GP_CombatComponent()
{
	PrimaryComponentTick.bCanEverTick = true;

	bPatternUseAble = true;
}

void UMonsterNormal_GP_CombatComponent::BeginPlay()
{
	Super::BeginPlay();

	check(elementalProjectile_Poison != nullptr);
	check(elementalProjectile_Frozen != nullptr);
	check(elementalOrb_Frozen != nullptr);
	check(elementalPillar_Poison != nullptr);
	check(elementalPillar_Frozen != nullptr);
}

void UMonsterNormal_GP_CombatComponent::TakeDamage(AActor* _damagedActor, float _damage, const UDamageType* _damageType, AController* _instigatedBy, AActor* _damageCauser)
{
...
void UMonsterNormal_GP_CombatComponent::Attack(EAI_MeleeAttack_State _state)
{
	Super::Attack(_state);
...
void UMonsterNormal_GP_CombatComponent::MeleeAttackGroup_1()
{
	...
	{
	case 0:
		monsterBase->MON_STATUS->SetElementalType(EAI_ElementalType_State::Poison);
	...
	case 1:
		...
		break;
	case 2:
		monsterBase->MON_STATUS->SetElementalType(EAI_ElementalType_State::Frozen);
		monsterBase->MON_STATEMACHINE->SetMagicSpawnTypeState(EAI_MagicSpawnType_State::Projectile);
...
void UMonsterNormal_GP_CombatComponent::SpecialSkill()
{
	Super::SpecialSkill();
...
void UMonsterNormal_GP_CombatComponent::LookAround(bool _stop)
{
	if (_stop)
...
void UMonsterNormal_GP_CombatComponent::SpawnElemental(FVector _fireLocation, FRotator _fireRotator, float _scale)
{
	...
	case EAI_MagicSpawnType_State::Projectile:
	{
		...
		if (monsterBase->MON_STATUS->GetElementalType() == EAI_ElementalType_State::Poison)
		{
			spawnProjectile = GetWorld()->SpawnActor<AMonsterElementalProjectile>(elementalProjectile_Poison, _fireLocation, _fireRotator, spawnParams);
			if (spawnProjectile)
				spawnProjectile->SetActorScale3D(FVector(_scale));
		}
		else if (monsterBase->MON_STATUS->GetElementalType() == EAI_ElementalType_State::Frozen)
		{
			spawnProjectile = GetWorld()->SpawnActor<AMonsterElementalProjectile>(elementalProjectile_Frozen, _fireLocation, _fireRotator, spawnParams);
			if (spawnProjectile)
				spawnProjectile->SetActorScale3D(FVector(_scale));
		}
		break;
	...
	case EAI_MagicSpawnType_State::Orb:
	{
		AMonsterElementalOrb* spawnOrb
			= GetWorld()->SpawnActor<AMonsterElementalOrb>(elementalOrb_Frozen, _fireLocation, _fireRotator, spawnParams);
		if (spawnOrb)
	...
	case EAI_MagicSpawnType_State::Pillar:
	{
		...
		if (monsterBase->MON_STATUS->GetElementalType() == EAI_ElementalType_State::Poison)
		{
			spawnPillar = GetWorld()->SpawnActor<AMonsterElementalPillar>(elementalPillar_Poison, _fireLocation, _fireRotator, spawnParams);
		}
		else if (monsterBase->MON_STATUS->GetElementalType() == EAI_ElementalType_State::Frozen)
		{
			spawnPillar = GetWorld()->SpawnActor<AMonsterElementalPillar>(elementalPillar_Frozen, _fireLocation, _fireRotator, spawnParams);
		}
		break;
	}
	default:
